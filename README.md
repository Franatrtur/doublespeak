...
These are all publicly available models on huggingface which can be passed as the model name into Doublespeak.

| Model Name | Recommended settings | Notes |
| :--- | :--- | :--- |
| **`gpt2`** | `end_bias=3`, `top_p=0.5` | Quick and small for plain text. Can sometimes loop repeating the same text over. |
