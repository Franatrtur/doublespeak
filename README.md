# DoubleSpeak
*words in disguise*  
__*LLM steganography*__

DoubleSpeak is an steganography tool that hides secret messages within natural-looking text generated by Large Language Models (LLMs).
...
*   **LLM-Based Encoding:** Uses Hugging Face `transformers` models to generate high-quality cover text.
*   **Nucleus Sampling Integration:** Encodes data by deterministically selecting tokens within the `top_p` probability mass.
*   **Stealthy Endings:** Configurable strategies (`natural`, `agressive`, `random`) to ensure the generated text ends plausibly, hiding the point where the message stops.
...
These are all publicly available models on huggingface which can be passed as the model name into Doublespeak. See the `/examples` folder for example runs, or behind the model name.
